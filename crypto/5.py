...
"""
for i in range(4):
    MK.append(eval(b'0x'+key[8*i:8*i+8]))
...
class PKCS(object):
    ...
    def fun(self):
        for i in range(4):
            self.context.append(eval(b'0x'+key[8*i:8*i+8]))
        """
        text_len=len(self.text)
        if text_len%16:
            self.text=self.text+(16-text_len%16)*chr(16-text_len%16)
        for i in range(4*(math.ceil(text_len/16))):
            self.context.append(eval(b'0x'+binascii.b2a_hex(bytes(self.text[8*i:8*i+8],encoding='utf-8'))))
...
"""
    ...
    x=''
    for i in range(4):
        x=x+hex(decry_text[i])[2:].zfill(8)
