...
class key(object):
    ...
    def replace1(self):
        ...
        lst=''
        for i in range(56):
            lst=lst+self.mk[PC_1[i]-1]       #PC2表中的序号是从1开始，所以需要-1？？？？？？？？？？？？
        #分割
        self.C=[lst[:28]]
        self.D=[lst[28:]]
        #print(lst[:28],lst[28:])
    ...
    def replace2(self,lst):
        lst2=''
        for i in range(48):
            lst2=lst2+lst[PC_2[i]-1]    #PC2表中的序号是从1开始，所以需要-1？？？？？？？？？？？？？？？
        return lst2
...
class decry(encry):
    def __init__(self,mk,data):
        #mk解密的子密钥（16个），data是要解密的密文（64比特）
        encry.__init__(self,mk,data)                                     #类的继承的调用？？？？？
        #super(decry,self).__init__(mk,data)
...
if __name__ == "__main__":
    ...
    """
    #####################################解密#########################
    E='85e813540f0ab405'         #密文
    decry_key=[]     #解密的子密钥
    #将加密的子密钥倒序就为解密的子密钥
    for i in range(16):
        decry_key.append(encry_key[-(i+1)])
    decry(decry_key,E).fun()    #解密
"""