...
class scanner(object):
    def __init__(self):
        self.content=read_file().line_content   #读文件
                ...
                if ch.isdigit():
                    ...
                    for i in token_table:
                        if i[1]==self.word.upper():
                            ...
                            flag=1
                    if flag==0:
                        item=scanner_Token('ERRTOKEN',self.word,0)
                    yield item                  
            else:
                #注释和运算符
                if ch  is '/':
...
def scanner_main():
    ...
    while True:
        if token.values=='@':
            ...
            print('%9s,%12s,%12s'%(token.type,token.values,token.lexeme))
